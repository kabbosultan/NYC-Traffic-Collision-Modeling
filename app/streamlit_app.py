# app.py - NYC KSI Prediction App (No Clustering)
...
# Patch for _RemainderColsList if needed
...
@st.cache_data
def load_meta():
    return json.load(open("ksi_model_meta.json"))
...
st.sidebar.caption(f"Trained on {meta['total_crashes']:,} NYC collisions")
...
with col2:
    ...
    if predict_button:
        try:
            # Prepare input for model - match EXACT feature names from training
            input_dict = {
                'hour': hour,
                ...
                'borough': borough,  # lowercase to match training
...
# Model insights section (no clustering needed)
...
with col3:
    ...
    st.markdown("""
    Based on our analysis of 342,000+ collisions, the model identified these key factors:
    
    **1. Pedestrian Involvement** 🚶
    - Increases severe injury risk by 350%
    - Most critical predictor of KSI outcomes
    
    **2. Cyclist Involvement** 🚴
    - Increases severe injury risk by 280%
    - Vulnerable road users at highest risk
    
    **3. Late-Night Crashes** 🌙
    - 11pm-5am crashes are 2.5x more likely to be severe
    - Alcohol, fatigue, and reduced visibility
    
    **4. High-Risk Behaviors** ⚠️
    - Alcohol, drugs, speeding, distraction
    - Doubles the probability of KSI outcomes
    
    **5. Multi-Vehicle Collisions** 🚗
    - 3+ vehicles involved increases severity
    - Complex crash dynamics and higher impact forces
    """)

with col4:
    st.markdown("### 📍 Borough Patterns")
    st.markdown("""
    **Manhattan:** Highest KSI rate (12%)
    - Dense pedestrian traffic
    - Late-night activity
    
    **Brooklyn:** Second highest (11%)
    - High traffic volume
    - Rush hour congestion
    
    **Queens:** Moderate risk (9%)
    - Mixed residential/commercial
    
    **Bronx:** Moderate risk (8.5%)
    - Major highway corridors
    
    **Staten Island:** Lowest (7%)
...
with st.expander("ℹ️ Understanding Model Metrics"):
    ...
    **Recall (78.5%)** tells us: "Of all severe crashes, we catch 78.5%"
    ...
    **Precision (45.2%)** tells us: "When we predict high risk, we're right 45.2% of the time"
    ...
    - But they're far better than missing a severe crash
    
    **The Trade-off:**
    - High recall = catch more severe crashes (good!)
    - But also = more false alarms (acceptable trade-off)
    - For Vision Zero: Better safe than sorry
    
    **Real-world impact:**
    If 100 collisions occur and 9 are severe (KSI):
    - Model catches **~7 of the 9 severe crashes** ✅
    - Model also flags **~8 minor crashes** as high-risk ⚠️
    - This means ~15 total high-risk alerts
    - Cost: Extra resources for 8 false alarms
    - Benefit: **7 lives potentially saved**
    
    **Vision Zero's perspective:** The cost of extra resources is worth saving lives.
    """)
